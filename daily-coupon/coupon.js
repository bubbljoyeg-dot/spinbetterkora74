document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentCouponCode = "SPINBETTER74";
    let currentCouponDesc = "قسيمة اليوم الحصرية، قم بنسخ الكود واستخدامه في الموقع.";
    ...
    async function fetchDailyCoupon() {
        try {
            ...
            console.warn("Could not fetch daily coupon, using fallback.");
        }
        
        codeDisplay.textContent = currentCouponCode;
        descDisplay.textContent = currentCouponDesc;
    }
    ...
    // Get voted state from localStorage
    function getVotedComments() {
        try { return JSON.parse(localStorage.getItem('kora74_coupon_votes')) || {}; } 
        catch (e) { return {}; }
    }
    
    function setVotedComment(id, type) {
        const votes = getVotedComments();
        votes[id] = type;
        localStorage.setItem('kora74_coupon_votes', JSON.stringify(votes));
    }
    ...
    async function loadComments() {
        try {
            ...
            if (!res.ok) {
                commentsList.innerHTML = '<div class="loading-comments">لا توجد قسائم مشتركة حتى الآن. كن أول من يشارك قسيمته!</div>';
                return;
            }
            ...
            data.forEach(comment => {
                ...
                const featuredBadge = comment.is_featured ? '<span class="featured-badge">⭐ اختيار الإدارة</span>' : '';
            ...
            commentsList.innerHTML = '<div class="loading-comments">تم إنشاء هيكل التعليقات. في انتظار ربط قاعدة البيانات...</div>';
        }
    }

    submitBtn.addEventListener('click', async () => {
        const name = authorInput.value.trim() || 'عضو مجهول';
        const body = bodyInput.value.trim();

        if (!body) {
            alert('الرجاء كتابة كود القسيمة أو التفاصيل.');
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert('حدث خطأ. تأكد من تفعيل جدول coupon_comments في لوحة التحكم.');
            }
        } catch (e) {
    ...
    window.voteComment = async function(id, type, btnElement) {
        ...
        // Since we can't easily increment without RPC in a safe way anonymously, 
        // we will fetch current and add 1 (Not perfectly safe for race conditions, but good for this setup)
        try {
