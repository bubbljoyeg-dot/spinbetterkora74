...
function updateMetaTags(post) {
    ...
    // which could be a dev server, old host, or still show news on cached deploys.
    const canonicalBase = 'https://kora74.online/news/';
...
function resetMetaTags() {
    ...
    if (ldEl) ldEl.remove();
    // Restore static og: defaults so the page og: reflects news, not the last article
    const set = (prop, val) => {
        const el = document.querySelector(`meta[property="${prop}"]`);
        if (el) el.setAttribute('content', val);
    };
    set('og:type',        'website');
    set('og:url',         'https://kora74.online/news/');
...
function updateNewsTicker() {
    ...
    const staticItems = [
        { title: '🔥 بونص ترحيبي 200% — استخدم كود W300 الآن!', url: '../promo-code/' },
        { title: '⚽ أفضل احتمالات الدوري الإنجليزي على kora74 اليوم', url: '../sports-betting/' },
        { title: '🏆 دوري أبطال أوروبا — راهن على أقوى المباريات', url: '../sports-betting/' },
        { title: '🎰 أكثر من 7,000 لعبة كازينو ومراهنات مباشرة', url: '../casino/' },
        { title: '💸 الإيداع والسحب بفودافون كاش وانستاباي بدون عمولات', url: '../deposit/' },
        { title: '🏆 بطولات يومية وجوائز نقدية فورية لكل اللاعبين', url: '../registration/' },
        { title: '📱 حمّل تطبيق kora74 على iOS وأجهزة الأندرويد', url: '../ios/' },
        { title: '🎯 توقعات MisterMedia لأهم مباريات الجولة القادمة', url: './' },
        { title: '🛡️ لعب آمن ومرخص — Curacao eGaming License', url: '../about/' },
        { title: '📊 kora74 — أعلى نسبة ربح في المراهنات الرياضية', url: '../sports-betting/' },
        { title: '🥇 الدوري المصري — الأهلي والزمالك — تابع التغطية الحصرية', url: './' },
        { title: '⚽ كرة القدم الأوروبية مباشرة — أهداف وحوادث لحظة بلحظة', url: './' },
        { title: '🎁 مكافآت Cashback أسبوعية حتى 15% على كل خسائرك', url: '../promo-code/' },
        { title: '📣 اشترك في النشرة البريدية — احصل على تحليلات حصرية', url: '../registration/' },
        { title: '🔔 تنبيهات فورية لأهم نتائج وأخبار الملاعب', url: './' },
        { title: '💰 أسرع عمليات سحب في المنطقة — خلال 5 دقائق فقط!', url: '../deposit/' },
        { title: '🏟️ دوري السيريا آ — مباريات اليوم وأفضل الاحتمالات', url: '../sports-betting/' },
        { title: '🇩🇪 البوندسليغا الألماني — بايرن ميونيخ في الصدارة', url: '../sports-betting/' },
        { title: '🇪🇸 لالیگا — ريال مدريد وبرشلونة — كلاسيكو بلا حدود', url: '../sports-betting/' },
        { title: '🎲 لعبة الحظ اليومية — ادخل واربح مع kora74', url: '../casino/' },
        ...
        ">
            ...
            <div style="padding:20px 20px 16px;text-align:center;border-bottom:1px solid rgba(255,255,255,0.05);">
                <a href="/" style="display:inline-block;margin-bottom:12px;">
                    <img src="../logo-kora74-official.png"
                         alt="Kora74"
